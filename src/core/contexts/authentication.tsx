  ...
  const verifyUserStatus = async (token: string) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/users/me?populate[tracks][fields][0]=title&populate[tracks][fields][1]=bpm&populate[tracks][fields][2]=moods&populate[tracks][fields][3]=keys&populate[tracks][populate][audio][fields][0]=url&populate[tracks][populate][cover][fields][0]=url&populate[soundKits][fields][0]=title&populate[soundKits][populate][cover][fields][0]=url&populate[soundKits][populate][tracks][populate][audio][fields][0]=url&populate[img][fields][0]=url&populate[track_purchased][fields][0]=id&populate[track_purchased][fields][1]=title`,
        {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        const requiresProducer = producerRoutes.some((route) =>
          pathname.startsWith(route)
        );
...
export const useAuthentication = () => {
  ...
  if (!context) {
    throw new Error(
      "useAuthentication must be used within an Authentication provider"
    );
  }
  return context;
};
